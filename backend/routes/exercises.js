...
router.post("/add", async (req, res) => {
  try {
    const { user_id, workout_id, intensity, exercise_type, calories_burned } =
      req.body;
    ...
    if (!user_id || !workout_id || !intensity || !exercise_type) {
      return res.status(400).json({
        error: "user_id, workout_id, intensity, and exercise_type are required",
      });
    }

    if (exercise_type != "cardio" && exercise_type != "strength training") {
      return res.status(400).json({
        error: "exercise_type must be 'cardio' or 'strength training'",
      });
    }
    ...
    if (exercise_type === "cardio") {
      if (!exercise_time) {
        return res
          .status(400)
          .json({ error: "cardio exercises must have an excercise_time" });
      }
    }
    ...
    if (calories_burned <= 0) {
      return res.status(400).json({
        error: "an excercise must have a positive int for calories_burned",
      });
    }
...
router.post("/update", async (req, res) => {
  try {
    ...
    if (!exercise_id || !intensity || !exercise_type) {
      return res.status(400).json({
        error: "exercise_id, intensity, and exercise_type are required",
      });
    }
    if (exercise_type !== "cardio" && exercise_type !== "strength training") {
      return res.status(400).json({
    ...
    if (calories_burned <= 0) {
      return res.status(400).json({
        error: "an exercise must have a positive int for calories_burned",
      });
    }
    ...
    if (exercise_type === "cardio") {
      ...
      if (!exercise_time) {
        return res
          .status(400)
          .json({ error: "cardio exercises must have an exercise_time" });
      }
    }
    ...
    if (exercise_type === "strength training") {
      ...
      if (!weight || !sets || !reps) {
        return res.status(400).json({
          error: "strength training exercises must have weight, sets, and reps",
        });
      }
    }
